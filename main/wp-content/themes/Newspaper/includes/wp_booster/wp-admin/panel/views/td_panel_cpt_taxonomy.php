<hr>
<div class="td-section-separator">自定义文章类型</div>
...
if ($are_custom_post_types_installed === false) {
    echo    '<div class="td-panel-no-settings-found" style="text-align: center">
                <strong>没有检测到自定义文章类型</strong> <br>
                <span>
                    请注意，WooCommerce文章类型已忽略
                </span>
            </div>
            ';
}
...
<hr>
<div class="td-section-separator">自定义分类法</div>
<?php
...
foreach ($td_taxonomies_obj as $td_taxonomy_obj) {
    ...
    $taxonomy_used_on_cpt_html = '<span class="td-box-title-right-title">用于CPT：</span>';
    if (!empty($td_taxonomy_obj->object_type)) {
        ...
        $taxonomy_used_on_cpt_html .= '<span class="td-box-title-label">不用于CPT</span>';
    }
