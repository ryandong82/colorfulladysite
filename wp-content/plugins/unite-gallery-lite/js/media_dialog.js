...
function UniteMediaDialogUG(){
	...
	function onChangePreviewImageClick(){
		...
		g_ugAdmin.openAddImageDialog(dialogTitle, function(arrData, imageID){
			if(arrData.length == 0)
				return(false);
	...
	function onChangeThumbImageClick(){
		...
		g_ugAdmin.openAddImageDialog(dialogTitle, function(arrData){
			if(arrData.length == 0)
				return(false);
	...
	function initEvents(){
		...
		jQuery(".check-video-type").click(function(){
		...
		//set search actions
		jQuery("#dv_button_youtube_search").click(searchYoutube);		
		jQuery("#dv_button_vimeo_search").click(searchVimeo);
		jQuery("#dv_button_wistia_search").click(searchWistia);
		
		jQuery("#dv_link_change_image").click(onChangePreviewImageClick);
		
		jQuery("#dv_link_change_thumb").click(onChangeThumbImageClick);

		jQuery("#dv_input_video_preview, #dv_input_video_thumb").change(updatePropsImages);

		jQuery(".dv_put_example").click(onPutExampleClick);
		
		//add the selected video to the callback function
		jQuery("#dv_button_video_add").click(onAddVideoClick);
	...
	function requestThumbUrl(){
		...
		g_ugAdmin.requestThumbUrl(urlImage, imageID, function(urlThumb){
...
g_ugMediaDialog = new UniteMediaDialogUG();
